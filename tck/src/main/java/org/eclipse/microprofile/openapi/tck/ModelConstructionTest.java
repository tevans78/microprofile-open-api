...
public class ModelConstructionTest extends Arquillian {
    ...
    public void apiResponsesTest() {
        ...
        assertSame(responses.getAPIResponse(responseKey), pathItemValue,
                "The value associated with the key: " + responseKey
                        + " is expected to be the same one that was added.");
        ...
        assertSame(responses.getAPIResponse(responseKey2), pathItemValue2,
                "The value associated with the key: " + responseKey2
                        + " is expected to be the same one that was added.");
    ...
    private <T extends Constructible> T createConstructibleInstance(Class<T> clazz) {
        ...
        assertTrue(clazz.isInstance(o1),
                "The return value of OASFactory.createObject() is expected to be an instance of: " + clazz.getName());
        ...
        assertTrue(clazz.isInstance(o2),
                "The return value of OASFactory.createObject() is expected to be an instance of: " + clazz.getName());
        assertNotSame(o2, o1, "OASFactory.createObject(" + clazz.getName()
                + ") is expected to create a new object on each invocation.");
        return o1;
    }
    ...
    private void processExtensible(Extensible<?> e) {
        ...
        assertEquals(map2, newMap,
                "The return value of getExtensions() is expected to be the same value that was set.");
    ...
    private void processReference(Reference<?> r) {
        // Check that the ref value can be set using the setter method and that the getter method returns the same
        // value.
    ...
    private <K, T> void checkMapEntry(Map<K, T> map, K key, T value) {
        ...
        assertSame(map.get(key), value,
                "The value associated with the key: " + key + " is expected to be the same one that was added.");
    }
